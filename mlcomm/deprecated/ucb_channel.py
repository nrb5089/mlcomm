...
class Agent:
    ...
    def update(self,y,t,idx):
        ...
        sigma = 0.01 #default worst case scenario
...
#            print(y)
            ...
            #Stopping criteria
            if self.Nts[It] > 1 + self.lam_p * (np.sum(self.Nts) - self.Nts[It]) and np.sum(self.Nts) > self.N:
#                print("CONVERGED!")
                for tc in range(tt,T):
                    if It in self.N_star and tt > self.N-1: Pc.append(1)
                    else: Pc.append(0)
                return np.array(Pc),P

            if It in self.N_star: Pc.append(1)
            else: Pc.append(0)
...
#    max_resolutions = [128]
    ...
    if True:
        ...
        for max_resolution in max_resolutions:
            ...
            for s in range(S):
                ...
                agent = Agent(channel,channel.sample_signal,max_resolution = max_resolution,delta_p = delta_p, mode = 'proc')
            ...
            if saveflg:
                ...
                f = open(path + filename + '_alg1_rician.pkl','wb')
    ...
    if False:
        ...
        for ii,max_resolution in enumerate(max_resolutions):
            filename = '/M_' + str(M) + '_N_' + str(max_resolution) + '_S_' + str(S) + '_T_' + str(T) + '_SNR_' + str(SNR) + '_alg1_rician.pkl'
