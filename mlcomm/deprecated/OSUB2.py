...
#        self.W = array_play_codebooks.create_codebook(M = Channel.M,N = max_resolution, Q = 128)
    ...
    def find_maxes(self):
        ...
        #max mu_hat in the entire graph
        for idx in range(len(nodes)):
            node_means.append(nodes[idx].mu_hat)
        max_mu_idx = np.argmax(node_means)    
        self.update_nuts(max_mu_idx)
        
        #max UCB in that neighborhood
    ...
    def update_nuts(self,max_idx):
        nodes = self.tree.branches
#        print(max_idx)
        if nodes[max_idx].indices[0] == self.l0:
            try:
                self.nodes_ut = np.unique(np.concatenate([[nodes[max_idx].sibling],
                                                          #[max_idx],
                                                          nodes[max_idx].cousin,
                                                          nodes[max_idx].children])).astype('int')
            except:
                print(nodes[max_idx].sibling)
                print(nodes[max_idx].cousin)
                print(nodes[max_idx].children)
        else:
            self.nodes_ut = np.unique(np.concatenate([[nodes[max_idx].parent],
                                                     #[max_idx],
...
#                self.gamma = 3                
            ...
            self.flops[t-1] += 2*self.channel.M -1 
