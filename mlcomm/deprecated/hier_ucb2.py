...
class Hier_UCB:
    ...
    def run_sim(self, T = 1000, bidx = 0, tol =2,track_flg = False, verbose = False):
        '''
...
#            if y < 0:
#                print('down')
            nodes[c_test].update(y,t,self.b,self.c,self.zeta/l_hat)
...
#    channel = cdl.CDL(signal_data = signal_data[s], M = M,kmax = max_data[s],SNR = SNR,seed = s) #-140.5 ~ 0 dB for SNR
    ...
    P,Pr = agent.run_sim(T =300,tol = tol,verbose = True)
    ...
    for ii in range(0,int(2*max_resolution -1)):
        ...
        LCBs.append(agent.tree.branches[ii].LCB)
