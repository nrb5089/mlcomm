...
def run_single_dm_demo(cb_graph):
    bandit = run_single_dm(1, {'snr_db' : 20, 'sigma_u' : .001, 'scenario' : 'LOS', 'cb_graph' : cb_graph})
...
def run_single_dm(seed,params_dict):
    ...
    bandit = ABT({'cb_graph' : cb_graph, 
                  'channel' : channel, 
                  'delta' : .01, 
                  'fading_estimation' : 'exact'})
    t0 = time()
...
def run_multi_parallel_dm(cb_graph,config_idx = 0):
    ...
    for scenario in ['LOS']:
        for snr_db in np.arange(20,51,5)[-1::-1]:
            for sigma_u in [.0005,.001,.005,.01]:
                params_dicts.append({'snr_db' : snr_db, 'sigma_u' : sigma_u, 'scenario' : scenario, 'cb_graph' : dcp(cb_graph)})
    ...
    for params_dict in params_dicts:
        ...
        for batch_idx in range(num_batches):
            ...
            pickle.dump({'rse_summed': rse_summed, 'rse_sq_summed' : rse_sq_summed,  'num_sims' : num_sims, 'runtime_summed' : runtime_summed, 'runtime_sq_summed' : runtime_sq_summed,'last_seed' : batch_seeds[-1]},open(f"./data/dm/snr_{params_dict['snr_db']}_sigma_u_{params_dict['sigma_u']}_scenario_{params_dict['scenario']}.pkl",'wb'))
    return out_summed/num_sims
