...
def run_single_ia(seed,params_dict):
    ...
    num_timesteps = 2000  #Arbitrary high time horizon, will terminate after finding beam.
...
def run_multi_parallel_ia(cb_graph,config_idx = 0):
    ...
    for snr_db in np.arange(10,31):
        for scenario in ['LOS','NLOS']:
            for fading_estimation in ['exact','estimate']:
    ...
    for batch_idx in range(num_batches):
        ...
        pickle.dump({'out_summed': out_summed, 'out_sq_summed' : out_sq_summed, 'num_sims' : num_sims, 'runtime_summed' : runtime_summed, 'runtime_sq_summed' : runtime_sq_summed, 'last_seed' : batch_seeds[-1]},open(f"./data/ia/snr_{params_dict['snr_db']}_fading_estimation_{params_dict['fading_estimation']}_scenario_{params_dict['scenario']}.pkl",'wb'))
