...
def main():
    ...
    run_multi_parallel_ia(cb_graph,int(config_idx))
...
def run_single_ia_demo(cb_graph):
    bandit = run_single_ia(2, {'snr_db' : 30, 'sigma_u' : 0, 'mode' : 'stationary',  'scenario' : 'LOS', 'time_horizon' : 75, 'cb_graph' : cb_graph})
...
def run_single_ia(seed,params_dict):
    ...
    time_horizon = params_dict['time_horizon']
    ...
    bandit =  HOSUB({'cb_graph' : cb_graph, 'channel' : channel, 'time_horizon' : time_horizon, 'starting_level' : 2, 'c' : 1, 'delta' : .01})
    t0 = time()
    bandit.run_alg()
...
def run_multi_parallel_ia(cb_graph,config_idx = 0):
    ...
    for snr_db in np.arange(10,31):
        for scenario in ['LOS','NLOS']:
            for time_horizon in [50,100]:
                params_dicts.append({'snr_db' : snr_db, 'sigma_u' : 0, 'time_horizon': time_horizon, 'scenario' : scenario, 'cb_graph' : dcp(cb_graph)})
    ...
    for params_dict in params_dicts:
        ...
        for batch_idx in range(num_batches):
            ...
            pickle.dump({'out_summed': out_summed, 'out_sq_summed' : out_sq_summed, 'num_sims' : num_sims, 'runtime_summed' : runtime_summed, 'runtime_sq_summed' : runtime_sq_summed, 'last_seed' : batch_seeds[-1]},open(f"./data/ia/snr_{params_dict['snr_db']}_budget_{params_dict['time_horizon']}_scenario_{params_dict['scenario']}.pkl",'wb'))
